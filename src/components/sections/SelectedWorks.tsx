    ...
    >
      <div className="card-top-row">
        <div className="id-brand-group">
          <span className="huge-number font-display">{project.id}</span>
          <div className="client-info">
            <span className="label font-display">{project.title}</span>
            <span className="client-name font-mono">{project.stack}</span>
          </div>
        </div>
      ...
      <div className="content-grid">
        <img
          src={project.image}
          className="main-image w-full h-auto object-contain bg-[#060608]/40"
          alt={project.title}
          onLoad={() => window.dispatchEvent(new Event("resize"))}
        />
        <div className="project-description font-mono">
...
export default function SelectedWorks() {
  ...
  // Map user's real projects to ScrollStackCard structure
  const formattedProjects = projects.map((p, index) => {
    const demoLink = p.links.demo || p.links.code || "#";
    const demoText = p.links.demoLabel || (p.links.demo ? "Ver Demo" : p.links.code ? "GitHub" : "Ver Proyecto");
    return {
      id: `00${index + 1}`,
      title: p.title,
      stack: p.stack.join(" / "),
  ...
  const cachePositions = useCallback(() => {
    ...
    if (endElement)
      endOffsetRef.current = endElement.getBoundingClientRect().top + scrollY;
    if (stackInnerRef.current)
      stackInnerTopRef.current =
        stackInnerRef.current.getBoundingClientRect().top + scrollY;
    setReady(true);
  }, [isMobile]);
  ...
  // Handle scroll tracking with custom scroll event listener
  useEffect(() => {
    ...
    const handleScroll = (inst: any) => {
      ...
      const scaleEndPositionPx =
        stackPositionPx -
        (BASE_CONFIG.stackPosition - BASE_CONFIG.scaleEndPosition) *
          containerHeight;
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        const triggerStart =
          cardTop - stackPositionPx - BASE_CONFIG.itemStackDistance * i;
        const triggerEnd = cardTop - scaleEndPositionPx;
        const pinStart = triggerStart;
        const pinEnd = Math.max(
          endElementTop - containerHeight * 0.5,
      ...
      if (kineticWheel) {
        if (
          scroll >
          endElementTop + containerHeight * 1.2 + containerHeight * 0.2
        ) {
          ...
          if (isMobile) {
            ...
            if (figureGroupRef.current) {
              if (voidProgress >= 0.8) {
                ...
                figureGroupRef.current.style.opacity = Math.max(
                  0,
                  textFade
                ).toFixed(3);
              } else {
  ...
  return (
    <section id="projects" className="min-h-screen bg-bg text-text font-mono relative select-none">
      <div className="w-full h-[20vh] border-b border-border/20 overflow-hidden flex items-center relative z-10 bg-bg">
        <div className="marquee-selected-works">
          <div className="marquee-selected-works__track">
            {[0, 1, 2, 3].map((blockIndex) => (
              ...
              >
                <span className="marquee-selected-works__text font-display tracking-tight uppercase text-accent font-bold">
                  Proyectos Destacados
                </span>
                <span className="marquee-selected-works__dash text-accent-2">—</span>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        {isMobile ? (
          ...
          >
            <defs>
              <linearGradient id="line-gradient" x1="0%" y1="0%" x2="0%" y2="100%">
                ...
                <stop offset="85%" stopColor="rgba(129, 140, 248, 0.7)" />
            ...
            <g ref={figureGroupRef}>
              ...
              <circle cx="750" cy="250" r="15" fill="#38bdf8" filter="url(#glow)" />
              ...
              <circle cx="250" cy="800" r="15" fill="#818cf8" filter="url(#glow)" />
              ...
              <circle cx="1250" cy="1350" r="15" fill="#a78bfa" filter="url(#glow)" />
              ...
              <circle cx="750" cy="1900" r="20" fill="#38bdf8" filter="url(#glow)" />
            </g>
          </svg>
        ) : (
          <svg
          ...
          >
            ...
            {[
              { text: "ANALYZE", offset: "15%", color: "#38bdf8" },
              { text: "●", offset: "27%", color: "#fff" },
              { text: "DESIGN", offset: "38%", color: "#818cf8" },
              { text: "●", offset: "50%", color: "#fff" },
              { text: "BUILD", offset: "62%", color: "#a78bfa" },
              { text: "●", offset: "73%", color: "#fff" },
              { text: "DELIVER", offset: "85%", color: "#38bdf8" },
            ].map((item, i) => (
              <text
                key={i}
                fill={item.color}
